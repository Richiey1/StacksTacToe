...
import { GameListSkeleton } from "@/components/ui/SkeletonLoaders";
...
export function GamesContent({
  ...
  const [selectedGameId, setSelectedGameId] = useState<bigint | null>(
    initialGameId || null,
  );
    ...
    async (gameId: bigint): Promise<Game | null> => {
      ...
      try {
        ...
        const isPlayerOneTurn = !!(
          safeVal(gameFields["is-player-one-turn"]) ?? true
        );
  ...
  const loadGames = useCallback(async () => {
    ...
    try {
      ...
      const totalGameCount =
        typeof latestIdData === "bigint"
          ? Number(latestIdData)
          : Number(latestIdData?.value || 0);
  ...
  const myActiveGames = address
    ? games.filter((g) => {
        ...
        const p1 = (
          typeof g.player1 === "string" ? g.player1 : ""
        ).toLowerCase();
        const p2 = (
          typeof g.player2 === "string" ? g.player2 : ""
        ).toLowerCase();
          ...
          {loading && games.length === 0 ? (
            ...
            <>
              {waitingGames.length > 0 && (
                <div className="mb-10">
                  <h2 className="text-xs font-pixel text-gray-400 mb-6 uppercase tracking-widest border-b border-white/10 pb-2">
                    Arena Lobby
                  </h2>
                  <GamesList
                    games={waitingGames}
              ...
              {games.length === 0 && !loading && (
                <div className="text-center py-20 border-4 border-dashed border-white/10">
                  <div className="text-gray-400 text-sm mb-6 font-pixel uppercase">
                    The Arena is Empty
                  </div>
                  <button
                    onClick={() => onTabChange?.("create")}
