...
export function LeaderboardContent() {
  ...
  const players = (leaderboardData || [])
    .map((player: any) => {
      ...
      const winRate =
        totalGames > 0 ? (wins / totalGames) * 100 : wins > 0 ? 100 : 0;
  ...
  if (isLoading) {
    return (
      <div className="px-4 py-8 md:px-8 max-w-6xl mx-auto space-y-8">
        <div className="h-20 w-full bg-zinc-900 border-4 border-white/10 rounded-xl animate-pulse" />
      ...
      <div className="px-4 py-8 md:px-8 max-w-6xl mx-auto relative">
        <div className="">
          <div className="text-center mb-10">
            ...
            <p className="text-gray-400 font-pixel text-xs uppercase tracking-tight">
              Top warriors ranked by performance
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
            ...
            <div className="border-4 border-white/10 bg-white/5 p-6 font-pixel text-center">
              ...
              <p className="text-2xl font-bold text-white">
                {formatEarnings(
                  players.reduce((sum, p) => sum + p.totalEarnings, 0),
                )}{" "}
                STX
              </p>
            </div>
