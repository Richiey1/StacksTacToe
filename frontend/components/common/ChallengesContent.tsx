...
export function ChallengesContent() {
  ...
  const allChallenges = (gamesData || [])
    .filter(game => game.status === 0 && !game.playerTwo) // Active games without player2 are open challenges
    .map(game => ({
      gameId: game.id,
      creator: game.playerOne,
  ...
  const handleAcceptChallenge = async (gameId: number) => {
    try {
      ...
      await joinGame(gameId, centerCell, challenge.betAmount);
